...
async def predictRouteClient(request: Request):
    try:
        ...
        cluster_number = int(predicted_cluster[0])
        ...
        segment_name = cluster_map.get(cluster_number, "Unknown Segment")

        final_output = f"{cluster_number}<br>Customer belongs to {segment_name}"

        return templates.TemplateResponse(
            "customer.html",
            {"request": request, "context": final_output}
        )
